...
export default function ImportTranscriptOption() {
  ...
  // If the user switches to local transcription while a picker/parse is in flight, invalidate
  // so a late onChange/parse cannot flip model back to import.
  useEffect(() => {
    return useEditorStore.subscribe((state, prev) => {
      if (!isTranscriptionModel(state.model) || state.model === prev.model) return;
    ...
    <>
      <ModelOption
        ...
        onSelect={(ctx) => {
          ...
          if (isTranscriptionModel(current)) {
