...
export default function MediaPreview() {
  const mediaUrl = useEditorStore((s) => s.playback?.url ?? null);
  const mediaName = useEditorStore((s) => s.manifest?.media.name ?? "Media");
  const mediaKind = useEditorStore((s) => s.manifest?.media.mediaKind ?? null);
  const words = useEditorStore((s) => s.words);
  const manualCuts = useEditorStore((s) => s.manualCuts);
  const duration = useEditorStore((s) => s.duration);
  const playing = useEditorStore((s) => s.playing);
  const currentTime = useEditorStore((s) => s.currentTime);
  const setVideoEl = useEditorStore((s) => s.setMediaEl);
